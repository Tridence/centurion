(function ( $ ) {
	...
	$( document ).on(
		'halstein_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'halstein_trigger_swiper_is_initialized',
							[$holder, options]
						);
...
(function ( $ ) {
	...
	$( document ).on(
		...
		function ( e, $holder, nextPage ) {
			qodefPagination.triggerLoadMore(
				$holder,
				nextPage
			);
		}
	...
	var qodefPagination = {
		...
		initPaginationType: function ( $holder ) {
			if ( $holder.hasClass( 'qodef-pagination-type--standard' ) ) {
				...
				qodefPagination.initInfiniteScroll( $holder );
			} else if ( $holder.hasClass( 'qodef-pagination-type--simple' ) ) {
				qodefPagination.initSimple( $holder );
			}
		},
		initStandard: function ( $holder, nextPage ) {
			...
			if ( $paginationItems.length ) {
				var options      = $holder.data( 'options' ),
					current_page = typeof nextPage !== 'undefined' && nextPage !== '' ? parseInt(
						nextPage,
						10
					) : 1;

				qodefPagination.changeStandardState(
					$holder,
			...
			if ( $paginationItems.length ) {
				...
				$paginationItems.children().each(
					function () {
						...
						qodefPagination.changeSimpleState(
							$holder,
							options.max_pages_num,
							1
						);
		...
		changeSimpleState: function ( $holder, max_pages_num, nextPage ) {
			if ( $holder.hasClass( 'qodef-pagination-type--simple' ) ) {
				var $paginationNav = $holder.find( '.qodef-m-pagination-items' ),
					$prevItem      = $paginationNav.children( '.qodef--prev' ),
					$nextItem      = $paginationNav.children( '.qodef--next' );

				$prevItem.children().data(
					'paged',
					nextPage - 1
				);

				if ( nextPage > 1 ) {
					$prevItem.removeClass( 'qodef-disabled' );
				} else {
					$prevItem.addClass( 'qodef-disabled' );
				}

				$nextItem.children().data(
					'paged',
					nextPage + 1
				);

				if ( nextPage === max_pages_num ) {
					$nextItem.addClass( 'qodef-disabled' );
				} else {
					$nextItem.removeClass( 'qodef-disabled' );
				}
			}
		},
		getNewPosts: function ( $holder, nextPage ) {
			...
			qodefPagination.setNextPageValue(
				options,
				nextPage,
				false
			);
			...
			$.ajax(
				{
					...
					success: function ( response ) {
						...
						if ( response.status === 'success' ) {
							...
							qodefPagination.changeStandardHtml(
								$holder,
								options.max_pages_num,
								nextPage,
							...
							qodefPagination.addPosts(
								$itemsHolder,
								response.data.html,
								nextPage
							);
							qodefPagination.reInitMasonryPosts(
								$holder,
								$itemsHolder
							);
							...
							qodefPagination.loadMoreButtonVisibility(
								$holder,
								options
							);
						} else {
...
(function ($) {
	...
	var qodefWooAddToCartButtons = {
		init: function () {
			var $button = $(
				'.qodef-woo-product-image-inner .button:not(.qqvfw-quick-view-button), ' +
				'.qodef-woo-product-image-inner .added_to_cart, ' +
				...
				$button.each(
					function () {
						var $thisButton = $(this),
							...
							buttonIcon = '<svg class="qodef-svg--button-arrow" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="17" viewBox="0 0 17 20" style="enable-background:new 0 0 17 20;" xml:space="preserve"><polyline class="qodef-m-arrow" points="7.1,3.9 13,9.8 7.1,15.9 "/><rect class="qodef-m-line"  width="1" height="20"/></svg>',
							buttonContent = '<span class="qodef-m-text">' + buttonText + '</span><span class="qodef-m-icon">' + buttonIcon + '</span>';
				...
				$holder.each(
					function () {
						...
						if ($button.length) {
							...
							$button.html('<svg class="qodef-svg--search" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="16px" height="18px" viewBox="0 0 16 18" xml:space="preserve"><circle cx="7.769" cy="7.35" r="6.663" /><line x1="11.211" y1="13" x2="14.432" y2="17.512" /></svg>');
						}
					}
				);
			}
