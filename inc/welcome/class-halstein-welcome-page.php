...
if ( ! class_exists( 'Halstein_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Halstein_Welcome_Page {
		private static $instance;

		/**
		 * @return Halstein_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_halstein_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_halstein_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_halstein_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'halstein_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'halstein' ),
				esc_html__( 'About', 'halstein' ),
				current_user_can( 'edit_theme_options' ),
				'halstein_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'halstein_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( HALSTEIN_ROOT_DIR . '/screenshot.png' ) ? HALSTEIN_ROOT . '/screenshot.png' : HALSTEIN_ROOT . '/screenshot.jpg';

			halstein_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_halstein_welcome_page' === $hook ) {
				wp_enqueue_style( 'halstein-welcome-page-style', HALSTEIN_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Halstein_Welcome_Page::get_instance();
}
